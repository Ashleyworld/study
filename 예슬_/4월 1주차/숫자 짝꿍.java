...
//주어진 코드는 주어진 문자열에서 각 숫자의 등장 횟수를 세어 배열에 저장하는 메서드입니다. 이 메서드는 문자열과 해당 문자열에서 숫자의 등장 횟수를 저장할 배열을 매개변수로 받습니다.

  //      설명을 상세히 하면:

    //    countNumInArr 메서드는 주어진 문자열과 해당 문자열에서 숫자의 등장 횟수를 저장할 배열을 매개변수로 받습니다.
    //  이 메서드는 문자열의 각 문자를 순회하면서 숫자를 찾습니다. for 루프에서는 i 변수를 이용하여 문자열을 순회합니다.
    // 문자열의 각 문자는 charAt(i) 메서드를 통해 가져오며, 이를 숫자로 변환하기 위해 아스키 코드를 이용합니다. 문자 '0'의 아스키 코드를 뺌으로써 해당 문자가 나타내는 숫자를 얻습니다. 예를 들어, '1'의 아스키 코드는 49이고, '0'의 아스키 코드는 48이므로 '1' - '0'은 1이 됩니다.
